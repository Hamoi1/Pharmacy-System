...
<div class="point-of-sales" wire:ignore.self>
    <div class="px-lg-5 px-3 mt-4 user-select-none {{  app()->getLocale() == 'ckb'  || app()->getLocale() == 'ar' ? 'reverse' : '' }}">
        ...
        <section class="invoice-section border-bottom pb-2">
            <div class="d-flex align-items-center gap-2 ">
                <button class="puls-invoice btn" wire:click="AddNewInvoce">
                    <i class="fa fa-plus"></i>
                </button>
                <div class=" d-flex align-items-center gap-2 invoices">
                    @foreach ($invoices as $invoicePage )
                    <button class="btn border rounded {{ $invoicePage == $invoice ? 'bg-dark': '' }}">
                        <span class="badge bg-primary fs-5 py-2" wire:click="$set('invoice','{{$invoicePage}}')">{{ $invoicePage }}</span>
                        <span class="cursor-pointer">
                            <i class="fa fa-trash {{  app()->getLocale() == 'ckb'  || app()->getLocale() == 'ar' ? 'me-3' : 'ms-3' }} mt-1 text-danger" wire:click="DeleteInvoicePage('{{ $loop->index }}')"></i>
                        </span>
                    </button>
                    @endforeach
                </div>
