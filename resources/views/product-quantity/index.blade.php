...
<div>
    <div class="{{ app()->getLocale() == 'ckb'  || app()->getLocale() == 'ar' ? 'reverse' : '' }} px-lg-5 px-3 ">
        <div class="row">
            ...
            <div class="col-md-3 col-12 mt-4">
                <span class="d-block mt-2">
                    {{ __('header.TotalQuantity').' : '.$product->quantity_product }}
                </span>
                <span class="d-block mt-2">
                    {{ __('header.sale_price').' : '. $product->sale_price_product }} {{ __('header.currency') }}
                </span>
            </div>
        </div>
        <div class="row mt-3">
        ...
        @include('product-quantity.add-update')
        @include('product-quantity.delete')
        <div class="table-responsive mt-3" wire:loading.remove wire:target="search,Category,Supplier,previousPage,nextPage,gotoPage">
            <table class="table table-vcenter table-nowrap">
                ...
                <tbody>
                    @foreach ($product->ProductsQuantity as $p )
                    <tr>
                        <td>
